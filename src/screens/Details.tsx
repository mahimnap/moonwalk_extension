...
import { saveLaunch } from "../helpers/SaveLaunch";
...
const Details: React.FC<Props> = ({ route }) => {
  ...
  const [saveLaunchTapped, setSaveLaunchTapped] = useState(false);
  ...
  const actionItems = [
    [
      ...
      {
        title: `Save this Launch`,
        icon: "ChevronRight",
        thumbIcon: saveLaunchTapped ? "StarFill" : "Star",
        thumbColor: "#f24b4b",
        disabled: saveLaunchTapped,
        action: () => {
          saveLaunch(data); 
          analytics().logEvent("SAVE_LAUNCH", { value: data.name });
          setSaveLaunchTapped(true);
        },
      }
    ],
  ];
